...
void got_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet) {
    ...
    sin.sin_addr = oip->iph_sourceip; // A little tricky

    struct ipheader *ip = (struct ipheader *) buffer;
    struct ficmpheader *icmp = (struct ficmpheader *)(buffer + sizeof(struct ipheader));
    ...
    ip->iph_ident = oip->iph_ident; // every id of ip packet is increment by 1 
    ip->iph_flag = 0;
    ip->iph_offset = 0;
    ip->iph_ttl = 64;
